<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
        <meta name="keywords" content="Khvedela, Khvedelasenpai, Nikoloz Khvedelidze">
        <meta name="description" content="Khvedela's main website">
        <meta name="author" content="Nicholas Khvedelidze">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="../stylesv2-0.css">
        <title>khvedela</title>
        <link rel="icon" href="../icons/logo.png">
        <script src="../myScript.js"></script>
    </head>
    <body id="body" onload="loadTheme()">
        <header>
            <br><br>
        </header>
        <nav id="navbar">
            <a class="nav-button" href="../index.php">Home</a>
            <a class="nav-button" href="../about-me.php">About Me</a>
            <a class="nav-button" href="#">Contact Me</a>
            <button class="nav-button-open" id="navButtonOpen" onclick="openSideNav()">Ξ</button>
        </nav>
        <div id="mySidenav" class="sidenav">
            <a class="nav-button" href="../upcoming-updates.html" title="Upcoming Updates">
                Upcoming Updates
            </a>
            <a class="nav-button-active" href="fatex.php" title="FateX discord community">
                FateX Discord
            </a>
            <a class="nav-button" href="discord.php" title="My Discord group">
                My Discord
            </a>
            <a class="nav-button" href="youtube.php" title="My Youtube channel">
                My Youtube
            </a>
            <button class="btn-toggle" onclick="toggleMode()">◐</button>
        </div>
        <main>
            <div class="div-container">
                <h1>FateX Discord</h1>
                <div class="div-box">
                    <span class="box-text">Information will be available soon!</span>
                </div>
                <!--
                <div class="div-box">
                    <span class="box-text">
                        FateX network is one of the best between Minecraft networks.<br>
                        They offer different gamemodes and a great <a href="">comunity</a>!<br>
                    </span>
                </div>
                <div class="div-box">
                <img src="../images/fatex_banner.png" alt="fatex banner">
                </div>
                <div class="div-box">
                    <span class="box-text">
                        You can play different gamemodes such as:<br>
                        Mythical PvP, Mythical Factions, Skyblock etc.
                    </span>
                </div>
                -->
            </div>
        </main>
        <footer>
            <p>&copy;Nicholas Khvedelidze. All rights reserved</p>
        </footer>
    </body>
</html>